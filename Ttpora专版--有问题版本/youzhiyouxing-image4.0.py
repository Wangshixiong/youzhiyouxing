...
def scrape_article_page(article_url, image_dir):
    """
    【模块一：爬取文章详情页 (v6.0 最终版)】
    【v6.0 核心转向】: 不再翻译为 Markdown，而是“搬运”原始 HTML，
    并对 <img> 和 <a> 标签执行“外科手术”，替换为本地路径。
    """
    ...
    for img_tag in all_images_in_element:
        ...
        if img_url:
            ...
            if local_filename:
                ...
                # 清理懒加载引入的多余 <span> 包装
                # (根据 693.html, 图片被 <span>...<span><img></span></span> 包装)
                if img_tag.parent and img_tag.parent.name == 'span':
                    if img_tag.parent.parent and img_tag.parent.parent.name == 'span':
                         # 替换掉最外层的 <span>
                        img_tag.parent.parent.replace_with(img_tag)
                    else:
                        # 替换掉单层 <span>
                        img_tag.parent.replace_with(img_tag)
                        
    # 4. 【v6.0 外科手术 - 超链接】
    # 查找【所有】的 <a> 标签，确保它们是绝对路径
    ...
    # 5. 【v6.0 最终搬运】
    # 获取容器内【所有】被修改过的 HTML
    # 这将保留所有 h2, p, em, strong, 和 p.copyright (原文链接)
    html_content = "".join(str(child) for child in content_body.children)

    # 6. 返回重建好的内容 (Markdown 标题 + 原生 HTML 正文)
